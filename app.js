...
function insertImage() {
    document.querySelectorAll('.box').forEach(image => {
        if (image.innerText.length !== 0) {
            if (image.innerText == 'pawn-w' || image.innerText == 'pawn-b') {
                image.innerHTML = `${image.innerText} <img class='all-img all-pown' src="images/${image.innerText}.svg" alt="">`
                image.style.cursor = 'pointer'
            }
            else {
                image.innerHTML = `${image.innerText} <img class='all-img' src="images/${image.innerText}.svg" alt="">`
                image.style.cursor = 'pointer'
            }
        }
    })
